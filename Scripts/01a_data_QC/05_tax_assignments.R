...
top10 <- read.delim("Raw_data/eDNA/12s/12s_e/2024_MiFish_E_12S_ASV_sequences.length_var.blast.out",
                    h=TRUE,
                    fill = TRUE) %>%
...
#ASVs where all 50 assignments are equal. means you have to manually blast.
w1 <- top10_occ %>%
  merge(.,max_ID, by = "ASV") %>%
...
#look up each ASV in 2024_12S_ASV_sequences.length_var.fasta, determine based on all top hits and region
...
fix_ASV <- read.csv(here::here("Processed_data", #read in fixed ASV-species combo
                               ...
                               "12s_e",
                               "LITassignment",
                               "problemASV_a_edited_2024.csv")) #added missing ASVs from end of script
...
f1 <- data.frame(lapply(f1, function(x) {
  gsub("Kareius bicoloratus x Platichthys stellatus", "Platichthys stellatuss", x) #out of range, more likely Greenland halibut 
  
}))

f1 <- data.frame(lapply(f1, function(x) {
  gsub("Platichthys stellatus x (Kareius bicoloratus x Platichthys stellatus)", "Platichthys stellatuss", x) #out of range, more likely Greenland halibut 
  
}))

f1 <- data.frame(lapply(f1, function(x) {
  gsub("Platichthys stellatus x Verasper variegatus", "Platichthys stellatuss", x) #out of range, more likely Greenland halibut 
...
#               "12s_e",
#             "LITassignment",
#            "top10_gbifid_higher_a_2024.csv"))
...
f2 <- merge(f1[c("ASV", "perc_ID", "species")], new_taxa[1:19], by.x = "species", by.y = "ids") %>% #might need to change number inside []
  select(-c("species")) %>%
...
r9$LCT <- c("Liparidae1")                            #add a group name (in order) for each row in r9
...
#Sebastolobus alascanus/altivelis
...
q6$LCT <- c("Sebastes", 'Anoplarchus insignis/purpurescens','Sebastolobus alascanus/altivelis', 'Platichthys stellatus', 
            'Pholis laeta/ornata', 'Sebastes', 'Oncorhynchus keta', 'Sebastes','Oncorhynchus mykiss' )         #add a group name (in order) for each row in q6
...
y1 <- spec_tax[c(1:23)] #changed these brackets from Ben's code... not sure if it will cause issues 
...
length(data$ASV) #342
length(unique(data$ASV)) #342
length(unique(top10_occ$ASV)) #342
...
#fix Sebastes names here#
data <- data.frame(lapply(data, function(x) {
  gsub("Sebastes nebulosus", "Sebastes", x) #out of range, more likely Greenland halibut 
  
}))

data <- data.frame(lapply(data, function(x) {
  gsub("Sebastes entomelas", "Sebastes", x) #out of range, more likely Greenland halibut 
  
}))

data <- data.frame(lapply(data, function(x) {
  gsub("Sebastes mystinus", "Sebastes", x) #out of range, more likely Greenland halibut 
...
top10 <- read.delim("Raw_data/eDNA/12s/12s_u/2024_12S_ASV_sequences.length_var.blast.out",
                    h=TRUE,
                    fill = TRUE) %>%
  `colnames<-`(c("ASV", "subject", "accesion_num", "taxa_ID", "perc_ID", "coverage", "evalue", "bitscore", "source", "taxonomy")) %>%
  as.data.frame() %>%
...
fix_ASV <- read.csv(here::here("Processed_data",
                               ...
                               "12s_u",
                               "LITassignment",
                               "problemASV_a_2024_edited.csv")) #added ASV 218- Sebastes
...
#Isopsetta isolepis, Parophrys vetulus, Psettichthys melanostictus - Pleuronectidae2
#Bathylagus pacificus, Pseudobathylagus milleri - Bathylagidae1


r9$LCT <- c("Pleuronectidae2", "Bathylagidae1")                            #add a group name (in order) for each row in r9
r10 <- merge(r8, r9, by = "all_species") %>%
  mutate(level = "family") %>%
  mutate(genus = LCT) %>%
  mutate(species = LCT) %>%
...
#Xiphister atropurpureus, Xiphister mucosus -> Xiphister atropurpureus/mucosus
...
q6$LCT <- c("Sebastes", "Sebastes", 'Liparis dennyi/fucensis', "Xiphister atropurpureus/mucosus","Oligocottus maculosus/snyderi", 
            'Anoplarchus insignis/purpurescens', "Gadus chalcogrammus", "Sebastes", "Sebastes")         #add a group name (in order) for each row in q6
