...
#               "12s_e",
#             "LITassignment",
#            "top10_gbifid_higher_b_2024.csv"))
...
q6$LCT <- c("Sebastes", 'Sebastes')         #add a group name (in order) for each row in q6
...
length(data$ASV) #56
length(unique(data$ASV)) #56
length(unique(top10_occ$ASV)) #56
...
fix_ASV <- read.csv(here::here("Processed_data",
                               ...
                               "12s_u",
                               "LITassignment",
                               "problemASV_b_2024_edited.csv")) #added ASV 218- Sebastes
...
new_taxa <- read.csv(here::here("Processed_data",
                                ...
                                "12s_u",
                                "LITassignment",
                                "12sutaxonomy_b_2024.csv"),
                     head=TRUE)  #import annotated taxa

#create dataframe with all top hits and accepted taxonomy 
#select tazonomy for max percent ID
f2 <- merge(f1[c("ASV", "perc_ID", "species")], new_taxa[1:15], by.x = "species", by.y = "ids") %>% #might need to change number inside []
  select(-c("species")) %>%
  rename("species" = "species.y")
...
q6$LCT <- c("Sebastes", "Xiphister atropurpureus/mucosus", "Sebastes")         #add a group name (in order) for each row in q6
...
write_csv(table,
          here("Processed_data",
               ...
               "12s_u",
               "LITassignment",
               "taxonomy_groups_12su_eDNA_b.csv"))
